	...
	return (
		<>
			<Dialog open={open} onClose={onClose}>
				...
				<DialogContent>
					<DialogContentText>Edit the form to give details about this report.</DialogContentText>
					...
					<TextField
						...
						onChange={(e) => {
							setFormData({
								...formData,
								title: preppend !== '' ? `${preppend} | ${e.target.value}` : e.target.value,
							});
						}}
					/>
					<TextField
				...
				<DialogActions>
					...
					<Button onClick={sendIssue} disabled={isLoading} color={'primary'}>
						{isLoading ? (
							<>
								<CircularProgress size="1rem" sx={{ mr: '0.5rem' }} /> {'Loading...'}
							</>
						) : (
							'Send'
						)}
